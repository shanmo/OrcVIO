...
namespace orcvio
{
          ...
          residual_weights(residual_weights_)
    {
        ...
        estimate_SE2_pose_flag = true; 
        // estimate_SE2_pose_flag = false; 
    }
    ...
    std::tuple<bool, Eigen::Matrix4d> ObjectFeatureInitializer::single_object_initialization_lite(std::shared_ptr<ObjectFeature> obj_obs_ptr, std::unordered_map<size_t, std::unordered_map<double, ClonePose>> &clonesCAM)
    {
        ...
        const int cam_id = 0;
        double first_timestamp = obj_obs_ptr->timestamps.at(cam_id).at(0);
        ClonePose anchorclone = clonesCAM.at(cam_id).at(first_timestamp);
        ...
        auto first_bbox = obj_obs_ptr->zb[0];
        auto bbox_lines = poly2lineh(bbox2poly(first_bbox));
        ...
        center(0, 0) = (first_bbox[0] + first_bbox[2]) / 2;
        center(1, 0) = (first_bbox[1] + first_bbox[3]) / 2;
