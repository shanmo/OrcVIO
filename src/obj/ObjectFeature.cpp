...
void ObjectFeature::create_class_to_kps_map()
{
    umap_class_to_kps["door"] = 4; 
    umap_class_to_kps["pylon"] = 5; 
    umap_class_to_kps["barrier"] = 8; 
    umap_class_to_kps["chair"] = 10; 
    umap_class_to_kps["diningtable"] = 12; 
    umap_class_to_kps["bicycle"] = 11; 
    umap_class_to_kps["car"] = 12; 
    umap_class_to_kps["tvmonitor"] = 8; 
}
    ...
    for (const auto & tracker : kp_trackers)
    {
        ...
        for (const auto& kp : tracker.second->kp_history) {
            ...
            else 
            {
                ...
                cv::line(img, prev_pt, curr_pt, kp_col, line_thickness);
            }
            prev_pt = curr_pt;
        }
        // plot the most recent kp 
        // circle(img, curr_pt, radius, kp_col, /*lineThickness=*/-.3);
        circle(img, curr_pt, radius, kp_col, -1.0);
    }
}
