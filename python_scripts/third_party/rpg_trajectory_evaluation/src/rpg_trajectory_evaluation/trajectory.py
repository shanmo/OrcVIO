...
class Trajectory:
    ...
    def _safe_remove_file(abs_rm_fn):
        if os.path.exists(abs_rm_fn):
            ...
            print(Fore.YELLOW + 'Warning: Cannot find file {0}'.format(abs_rm_fn))
    ...
    def write_kitti_errors_to_yaml(self):
        ...
        trans_rmse = self.abs_errors['abs_e_trans_stats']['mean']
        ...
        for dist in self.rel_errors:
            cur_err = self.rel_errors[dist]
            if cur_err['rel_trans_stats'] is not None:
                valid_count += 1
                trans_percentage += cur_err['rel_trans_perc_stats']['mean']

        trans_percentage /= (valid_count + 1e-5)

        stats = dict()

        label = "RMSE(m)"
        stats[label] = trans_rmse

        label = "TransError(%)"
        stats[label] = trans_percentage

        with open(dist_fn, 'w') as outfile:
